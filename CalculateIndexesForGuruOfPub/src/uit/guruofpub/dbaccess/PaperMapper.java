...
public class PaperMapper extends MapperDB {
    ...
    public int[] checkExistingStatusOfPublication(String strPub) throws Exception {        
        ...
        try {
            ...
            sql.append(" SELECT * FROM guruofpub.paper p");
    ...
    public int insertObj(PaperDTO paperDTO) throws Exception {
        try {
            StringBuffer sql = new StringBuffer();
            sql.append("INSERT INTO guruofpub.paper(doi, isbn, url, title, abstract, volume, pages, year, viewPublication, bibTex, endNote, idJournal, idConference, idMagazine, version, paperFile) ");
            sql.append(" VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    public int updateObj(PaperDTO paperDTO) throws Exception {
    	 try {
    	     StringBuffer sql = new StringBuffer();
             sql.append("UPDATE guruofpub.paper");
    ...
    public ArrayList searchPaper(String[] keywords, int limit) throws Exception {
        ...
        try {
            StringBuffer sql = new StringBuffer();
            sql.append(" SELECT * FROM guruofpub.paper p");
            sql.append(" join guruofpub.author_paper ap on p.idPaper = ap.idPaper");
            sql.append(" join guruofpub.author a on ap.idAuthor = a.idAuthor");
            sql.append(" join guruofpub.conference c on p.idConference = c.idConference");
            sql.append(" join guruofpub.journal j on p.idJournal = j.idJournal");            
    ...
    public ArrayList getAuthorDTOList(int idPaper) throws Exception {
        ...
        try {
            StringBuffer sql = new StringBuffer();
            sql.append(" SELECT distinct a.* FROM guruofpub.paper p join guruofpub.author_paper ap on p.idPaper = ap.idPaper");
            sql.append(" join guruofpub.author a on ap.idAuthor = a.idAuthor");
            sql.append(" where p.idPaper = ?");
            PreparedStatement stmt = getConnection().prepareStatement(sql.toString());
            stmt.setInt(1, idPaper);
    ...
    public int getCitationCount(int idPaper) throws Exception {
        ...
        try {
            StringBuffer sql = new StringBuffer();
            sql.append(" SELECT rp.citationCount FROM guruofpub.paper p join guruofpub._rank_paper rp on p.idPaper = rp.idPaper");
            sql.append(" where p.idPaper = ?");
            PreparedStatement stmt = getConnection().prepareStatement(sql.toString());
            stmt.setInt(1, idPaper);
            ResultSet rs = stmt.executeQuery();
